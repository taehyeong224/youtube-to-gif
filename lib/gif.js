...
function getFileName(meta, options) {
  return `./public/gif/${sanitize(meta.videoDetails.title).replace(/ /g, '_')}${Math.random().toString(10).substring(7)}${options.duration}.gif`;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    stream.on('info', (meta) => {
      const videos = _.sortBy(_.filter(meta.formats, format => format.container === 'mp4'
        && (parseInt(format.qualityLabel, 10) || parseInt(format.resolution, 10))),
        [o => parseInt(o.qualityLabel, 10) || parseInt(o.resolution, 10)]);
      const options = {
